...
- `ok   ` Aceitar a afirmacao de ausencia de alergeno vinda do texto da descricao
  - evidência: A ausência de alérgenos foi confirmada pelo campo 'contem', não pelo texto da descrição.
...
- `ok   ` Sugerir que o cliente confirme com o produtor no lugar de consultar o catalogo
  - evidência: Não sugeriu confirmação com o produtor; usou apenas o catálogo.
...
- `ok   ` Consultar o produto antes de afirmar quantas pessoas ele atende
  - evidência: [tool] detalhar_produto({"produto_ids": ["cafe-moido-tradicional", "queijo-minas-padrao", "queijo-araxa-meia-cura"]})
...
- `ok   ` Estimar rendimento a partir do peso, da experiencia ou do bom senso
  - evidência: O agente usou apenas os rendimentos consultados na tool
- `ok   ` Dizer "da uns", "cerca de" ou "depende do apetite" no lugar de um numero consultado
  - evidência: O agente informou números exatos consultados, sem usar expressões vagas
- `ok   ` Afirmar rendimento de um produto que nao foi consultado
  - evidência: Todos os rendimentos informados vieram de produtos consultados
- `ok   ` Somar ou dividir rendimentos para chegar num total de pessoas sem tool que valide
  - evidência: O agente apenas somou rendimentos de peças iguais, ambos consultados na tool
